...
tinymce.PluginManager.add('imageplus', function(editor) {
	...
	function showDialog(imageList) {
		...
		function uploadComplete() {
			...
			getImageSize('/'+ data.directory + data.large, function(data) {
				if (data.width && data.height && imageDimensions) {
					width = data.width;
					height = data.height;
					
					if(typeof(editor.settings.image_auto_resize_width) != 'undefined') {
						if(width >= editor.settings.image_auto_resize_width) {
							width = editor.settings.image_auto_resize_width;
							height = Math.floor((width / data.width) * data.height);
						}
					}
			...
			win.find('#src').value('/'+ data.directory + data.large);
		}

		function upload(fileNode, progressNode) {
			for(var i = 0; i < fileNode.files.length; i++) {
				var file = fileNode.files[i];
		...
		// binding upload function
		uploaderBinded = false;
		...
		function bindUploader(element) {
			...
			var actionNode = $('#'+ element._id +'-action');
			var actionParentNode = actionNode.parent();
			
			var progressNode = $('<div>').css('width', 0).css('height', '2px').css('position', 'absolute').css('background', '#3ad531').css('top', 0);
			var fileNode = $('<input type="file" />');
			fileNode.css('width', actionParentNode.width()).css('height', actionParentNode.height());
			fileNode.css('position', 'absolute').css('top', 0).css('opacity', 0);
			fileNode.on('change', function() {
				upload(fileNode.get(0), progressNode.get(0));
			});
			
			var bodyId = win.$el.attr('id');
			var body = $('#'+ bodyId +'-body');
			
			progressNode.appendTo(body);
			fileNode.appendTo(actionParentNode);
